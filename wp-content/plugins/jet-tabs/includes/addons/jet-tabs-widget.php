		...
		switch ( $item[ 'content_type' ] ) {
			case 'template':
				if ( ! empty( $item['item_template_id'] ) ) {
					...
					if ( ! $ajax_template && $template_cache_settings['enable'] && ! $in_elementor ) {
						$transient_key = md5( "jet-tabs-template-id-{$template_id}" );
						$cache_expiration_timeout = $this->get_cache_timeout( $template_cache_settings['cacheExpiration'] );
						...
						if ( ! empty( $transient_data ) ) {
							$template_content = $transient_data;
						} else {
							$template_content = $elementor_template->prepare_cached_template_data( $template_id );
							jet_set_transient( $transient_key, $template_content, $cache_expiration_timeout, $template_id, 'elementor_library' );
						}
					} else {
						$template_content = jet_tabs()->elementor()->frontend->get_builder_content_for_display( $template_id, true );
					}
					
					if ( ! empty( $template_content ) ) {
						if ( ! $ajax_template ) {
							if ( $template_cache_settings['enable'] && ! $in_elementor ) {
